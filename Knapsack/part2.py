...
sys.path.append('/Users/kapil_tahiliani/Machine Learning/ABAGAIL/ABAGAIL.jar')
...
for t in range(numTrials):
    ...
    fit = FixedIterationTrainer(rhc, 1)
    times =[0]
...
for t in range(numTrials):
    for CE in [0.95]:
        ...
        fit = FixedIterationTrainer(sa, 1)
        times =[0]
        for i in range(0,maxIters,1):
            start = clock()
            fit.train()
...
for t in range(numTrials):
    for popu,mate,mutate in product([100],[50],[50]):
        ...
        fit = FixedIterationTrainer(ga, 1)
        times =[0]
        for i in range(0,maxIters,1):
            start = clock()
            fit.train()
...
#MIMIC
for t in range(numTrials):
    for samples,keep,m in product([300],[150],[0.9]):
